...
def collate_batch(batch, pad_token_id: int, label_pad_id: int = -100):
    ...
    input_ids = torch.tensor([pad(ids, pad_token_id, max_len) for ids in input_ids_list], dtype=torch.long)
    attention_mask = torch.tensor([pad(am, 0, max_len) for am in attention_list], dtype=torch.long)
    labels = torch.tensor([pad(lab, label_pad_id, max_len) for lab in labels_list], dtype=torch.long)
